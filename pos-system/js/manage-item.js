$(document).ready(function () {
    ...
    $(document).on("click", ".delete-item", function () {
        ...
        $.ajax(ajaxConfig).done(function (data,textStatus, jqxhr) {
            ...
            newItem();
            if(jqxhr.status==500){
                alert("This Item Already Has Orders");
            }
            if(jqxhr.status===404){
                alert("No Item Who has Id "+custId);
            }
        });
    }else{
            alert("OK");
...
function loadItem() {
    ...
    $.ajax(ajaxConfig).done(function (items, textStatus, jqxhr) {
        $('#tbl-item').DataTable().destroy(false);
...
function rowUpdate(row) {
    ...
    var ajaxConfig = {
        method: 'PUT',
        url: 'http://localhost:8080/api/v1/items/' + $("#id").val(),
        async: true,
        data: itemData,
        contentType: "application/json"
    };
    $.ajax(ajaxConfig).done(function (response,textStatu,jqxhr) {
        if(jqxhr.status===204){
